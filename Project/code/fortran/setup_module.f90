...
! Purpose: Reads initialization file and sets all runwide parameters
...
module setup_module
  ...
  integer, save :: N  ! Number of masses connected by springs in the system
  real (dp), save :: alpha, C ! Constants to be defined
...
contains
  ...
  subroutine problemsetup_Init(inFile)
    ...
    call read_initFileInt(inFile,'num_masses',N)
    ! Read the nonlinear factor
    call read_initFileReal(inFile,'nonlinear_factor',alpha)
    ! Read the correction factor
    call read_initFileReal(inFile,'correction_factor',C)   
